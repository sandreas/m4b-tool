    ...
    public function writeTag(SplFileInfo $file, Tag $tag, Flags $flags = null)
    {
        if ($this->detectFormat($file) === static::FORMAT_MP4) {
            ...
            if($this->tone->isInstalled()) {
                ...
                // write only description and recording date with mp4v2 because tone / atldotnet has a bug (https://github.com/Zeugma440/atldotnet/issues/155)
                if($tag->year != null || $tag->description != null) {
                    $workaroundTag = new Tag();
                    $workaroundTag->year = $tag->year;
                    $workaroundTag->description = $tag->description;
                    $this->mp4v2->writeTag($file, $workaroundTag, $flags);
                }
            } else  {
