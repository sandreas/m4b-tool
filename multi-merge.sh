...
for D in `find $INPUT_DIR/*/*/ -maxdepth 1 -type d`
do
...
#	exit;
        ...
        if [ "1" == "$ADJUST_FOR_IPOD" ]; then
            $M4B_TOOL merge --auto-split-seconds=$AUTOSPLIT_SECONDS --adjust-for-ipod --name "$TITLE" --album="$TITLE" --audio-channels $AUDIO_CHANNELS --audio-bitrate $AUDIO_BITRATE --audio-samplerate $AUDIO_SAMPLERATE --audio-codec libfdk_aac "$D" --output-file="$CURRENT_OUTPUT_FILE" && echo "$NAME" >> multi-merge.log
        else
            $M4B_TOOL merge --auto-split-seconds=$AUTOSPLIT_SECONDS --name "$TITLE" --album="$TITLE" --audio-channels $AUDIO_CHANNELS --audio-bitrate $AUDIO_BITRATE --audio-samplerate $AUDIO_SAMPLERATE --audio-codec libfdk_aac "$D" --output-file="$CURRENT_OUTPUT_FILE" && echo "$NAME" >> multi-merge.log
        fi
